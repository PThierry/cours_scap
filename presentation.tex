...
\begin{frame}[t]
  ...
  \begin{itemize}
   ...
   \item L'état de l'art cible principalement l'écosystème open-source dans l'optique
     de permettre aux étudiants de comparer
  \end{itemize}
\end{frame}
...
\begin{frame}[t]
  ...
  \begin{block}{Un sujet complexe...}
  \begin{itemize}
    \item De plus en plus souvent fournies par les fournisseurs de logiciels
    \item Peuvent nécessiter une refonte de l'usage des logiciels
      \begin{itemize}
        \item utilisation laxiste du compte administrateur
        \item accès direct aux API internes sans filtrage
      \end{itemize}
    \item guides de plus en plus nombreux et contextualisés (ANSSI, DISA, NIST..)
    \item peut être fortement impactant sur l'architecture du SI
    \item ne doit jamais être sous-estimé
      \begin{itemize}
        \item en coût financier, en temps
        \item en impact humain
      \end{itemize}
    \item ne doit pas être oblitéré pour autant
  \end{itemize}
  \end{block}
...
\begin{frame}[t]
  ...
  \begin{block}{}
  \begin{itemize}
    \item Nommée PSSI, est sous la responsabilité du DSSI de
      l'entreprise, et rédigée en comité avec des experts en sécurité des
      systèmes d'information
    \item N'est valable que si elle est applicable et contrôlable
    \item Se met à jour avec la vie du système d'information
    \item Contient des éléments techniques et organisationnels
      \begin{itemize}
        \item choix des logiciels
        \item durcissement des configurations
        \item gestion des accès aux salles machines, au bâtiments
      \end{itemize}
    \item Dépend des moyens que la société accepte de mettre dans la
      sécurité de son système d'information (au sens large), mais aussi de la
      réglementation en vigueur\footnote{En France, il existe plusieurs réglementations
...
\begin{block}{Bastille}
  \begin{itemize}
    \item Vieil outil de durcissement de configuration,
    ...
    \item Basé sur une interface interactive pour le durcissement des
      ...
      de base
    \item En perte de vitesse face à des outils comme Lynis ou OpenSCAP pour l'audit et la
      remédiation
  \end{itemize}
\end{block}
\end{frame}
...
\begin{frame}[t]
  ...
  \begin{block}{lynis}
  \begin{itemize}
    \item Est un outil en ligne de commandes bien abouti
    \item permet d'étudier l'état des configuration et des versions des logiciels du
      système d'exploitation afin de les confronter à une politique de
      sécurité donnée, pouvant être personnalisée
    \item Fournit un rapport de bonne facture et donne un bon apperçu du
      niveau de durcissement de l'OS
    \item Supporte un grand nombre de tests, avec une synthèse très convenable
  \end{itemize}
  \end{block}
  }
  \only<2>{
...
\begin{frame}[t]
  ...
  \only<1>{
  \begin{block}{Un écosystème désordonné...}
  \begin{itemize}
    \item L'écosystème de gestion de la sécurité est
      longtemps resté siloté entre chaque industriel (Microsoft, Cisco, etc.)
    \item Chacun fournissait sa propre suite, propriétaire
    \item Les outils de validation des politiques de sécurité ont également
      longtemps implémenté leurs propres technologies pour valider les
      politiques standards, comme le PCI-DSS ou les exigences NIST
  \end{itemize}
  \end{block}
  \begin{block}{...source de problèmes}
    \begin{itemize}
      \item Les structures verticales multiples ont rendu le gestion de la sécurité
        d'un parc difficle à valider efficacement
      \item L'indépendances des échelles, des outils et des protocoles
        ...
        Center) du système d'information
    \end{itemize}
  \end{block}
  }

  \only<2>{
  \begin{block}{Comment corriger le tir ?}
  \begin{itemize}
    \item Formaliser l'identification des équipements logiciels, matériels
    ...
    \item Définir un ensemble de langages pour définir, sans prendre
  ...
  \begin{alertblock}{Contrainte}
    Faire participer l'ensemble des constructeurs et éditeurs au nouvel écosystème
  \end{alertblock}
  }
\end{frame}
...
\begin{frame}[t]
    \only<1>{
  \begin{block}{La création du standard}
  \begin{itemize}
    \item débute au début des années 2000, sur une demande du DoD
    \item le but est d'unifier les mécanismes de gestion de la
      sécurité des systèmes d'information autour d'un ensemble de standards
    \item doit permettre de s'appliquer à l'ensemble du SI (postes de travail,
      serveurs, appliances, équipements divers)
    \item Est finalement poussée par le DoD, la NSA, le DoHS
      et le DISA (Agence pour les systèmes d'informations de Défense)
    \item le standard est porté par le NIST (National Institute for Standard \&
      Technology)
  \end{itemize}
  \end{block}
  }
...
\begin{block}{Le CVSS}
  ...
  \end{itemize}
\end{block}
  \begin{exampleblock}{CVSS par la pratique}
      On associe souvent à l'échelle de CVSS l'échelle risque statistique
...
\begin{block}{Les CWE}
  \begin{itemize}
    \item Définit des familles de faiblesses (à différencier d'une vulnérabilité,
     une faiblesse n'impliquant pas nécessairement une vulnérabilité)
    \item Permet de fournir un indiquateur de qualimétrie de
      sécurité de la cible
    \item Sont organisée sous forme d'une structure hiérarchique de faiblesses
    \item Sont très nombreuses (il existe des centaines de CWE, classées et numérotées)
      \begin{itemize}
    ...
    \item A chaque identitfant de CWE est indiqué l'impact probable d'une CVE
      basée sur celle-ci et les manières de s'en prémunir
  \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[t]
  \begin{block}{Les CPE}
  \begin{itemize}
    \item Formalisme pour identifier de manière unique une plateforme
    \item Définit (v2.3) une structure de donnée hiérarchique,
      séparée par des ':'
      \begin{itemize}
        \only<1>{
      ...
      \end{itemize}
    \item Peut être enrichit localement, au format OVAL, sans déclarer son produit auprès du MITRE
  \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[t]
  ...
  \begin{block}{XCCDF est riche et complexe}
    \begin{itemize}
      \item Possède des mécanismes de contrôle intégrés (macros if)
      \item Permet d'intégrer des mécanismes d'interactivité via le formalisme
         OCIL (Open Checklist Interactive Language)
       \item Supporte la gestion de variables pouvant être rafinées selon
         les profils associés au guide de sécurité
    \end{itemize}
  \end{block}
\end{frame}
    ...
    \begin{itemize}
      \item SCAP est aujourd'hui supporté par diverses outils et fournisseurs de logiciels de sécurité
       ...
       \item Bien que SCAP soit basé sur un ensemble de standards, les fiches OVAL ou les STIGs ne sont pas toujours interopérables
          \begin{itemize}
            \item Cetains éditeurs, comme CIS, enrichissent le langage
...
\begin{alertblock}{Etat des lieux du projet}
  \begin{itemize}
    ...
    \item Le support des STIGs n'est pas toujours complet
  \end{itemize}
\end{alertblock}
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Frame 9/10 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
  \begin{block}{via un serveur de sécurité}
  \begin{itemize}
    \item Qui stocke la ou les STIGs
    \item Qui émet de manière périodique une vérification des équipements
      \begin{itemize}
        \item Conformité au(x) STIG(s)
        \item État des vulnérabilités
        \item ...
      \end{itemize}
    \item Qui consolide les résultats et les transmet en base pour traitement
  \end{itemize}
\end{block}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Frame 9/10 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
  \frametitle{Remédiation à chaud}
  \begin{block}{}
  \begin{itemize}
    \item Reste une action complexe est risquée
    \item Est basée, pour OpenSCAP, sur les outils de gestion de configuration
      \begin{itemize}
          \item Ansible
          \item Puppet
          \item ...
      \end{itemize}
    \item Peut être appliqué au moment de la vérification de la cible
  \end{itemize}
\end{block}
  \begin{alertblock}{}
  Reste une action souvent manuelle après remise du rapport sur la cible
\end{alertblock}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Frame 9/10 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
  \begin{block}{Formalisme OVAL}
    \begin{itemize}
      \item Est une définition, à laquelle on associe un ou plusieurs tests
      \item Peut être de type:
	      \begin{itemize}
	      ...
	      \end{itemize}
      \item Possède un identifiant unique (dénoté {\it id}), une description, un titre et une ou plusieurs cible(s)
    \end{itemize}
  \end{block}
  \begin{block}{Critères de validation de la fiche}
    \begin{itemize}
      \item Une fiche est vraie lorsque :
	      \begin{itemize}
...
\begin{frame}[t]
  \begin{block}{Les critères OVAL}
    \begin{itemize}
      \item Utilisent une logique booléenne récursive
      \item Permet via une mécanique booléenne de construire des fiches complexes
	      \begin{itemize}
		      ...
		      \item etc.
	      \end{itemize}
      \item Permettent de hiérarchiser les fiches, une fiche pouvant enrichir (et donc dépendre) d'une autre
    \end{itemize}
  \end{block}
  \begin{alertblock}{Attention à la complexité}
...
\begin{frame}[t]
  \begin{block}{Les sélecteurs OVAL}
    \begin{itemize}
	    ...
	    \item Afin de ne pas multiplier les fiches OVAL, le langage permet l'usage de variables, qu'on dénote sélecteur
    \end{itemize}
  \end{block}
...
\begin{frame}[t]
	...
	\begin{block}{Principe d'une checklist XCCDF}
		\begin{itemize}
			\item Permet de générer autant de STIGs que de profils définit dans la checklist
			\item Permet de valider la conformité d'une cible à un STIG donné
		\end{itemize}
	\end{block}
	}
	\only<2>{
	\begin{block}{une checklist XCCDF}
		\begin{itemize}
			\item Génère les STIGs et les checklist associées
...
\only<3>{
	...
	\begin{itemize}
		...
		\item Les fiches XCCDF définissant un ensemble de règles à
                  respecter, elles sont sous la responsabilité du RSSI en
                  entreprise (si celle-ci possède sa propre PSSI) et sous la
                  responsabilité d'organismes standardisateurs ou fournisseurs
                  de bonnes pratiques (NIST, ANSSI, etc.)
                \item la mise en application des fiches XCCDF (période,
                  méthode, rôles associés) est sous la responsabilité du RSSI
                  via la PSSI. Il en va de même pour la réponse à un incident
                  de sécurité (des bonnes pratiques existes cependant)
	\end{itemize}
	\end{block}
}
\end{frame}
...
\begin{frame}[t]
  \frametitle{Conclusion}
    ...
    \begin{itemize}
      \item SCAP est un écosystème riche est complexe. Il définit à la fois
      \begin{itemize}
        \item Des langages (OVAL, XCCDF...)
        \item Des dictionnaires (CVE, CWE, CPE, etc.)
        \item Des workflows
      \end{itemize}
    \end{itemize}
    \end{block}
    \begin{exampleblock}{Les plus}
      \begin{itemize}
        \item Il casse les silots liés aux solutions propriétaires
        \item Il permet d'unifier la gestion de la sécurité des SI
          en y associant tous les acteurs (industriels, éditeurs)
        \item Il fournit un standard ouvert (pas de brevets)
      \end{itemize}
    \end{exampleblock}
    \begin{alertblock}{Les moins}
      \begin{itemize}
         \item Il ne résout pas toujours en soi l'applicabilité des PSSI (gestions des images des conteneurs, etc.)
         \item Les différentes implémentations du standard ne sont pas toujours compatibles
      \end{itemize}
    \end{alertblock}
\end{frame}
